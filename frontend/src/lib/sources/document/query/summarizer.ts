...
async function summarizeChunk(chunkText: string, index: number): Promise<string> {
  const systemPrompt = `Summarize the following section of a document.
Include the key points, data, tables, and topics discussed. Keep it concise.
...
export async function mapReduceSummary(
  ...
  if (!chunks.length) {
    return "No indexed content available to summarize. Please process the document first.";
  }
  ...
  const systemPrompt = `You are provided with chronological summaries of different sections of a document.
Create a comprehensive, cohesive, and structured final summary of the entire document.
Use headings and bullet points where appropriate.
Do not add extra information.`;

  const userPrompt = `Write the entire summary in ${language} only — even if the document segments are in another language.

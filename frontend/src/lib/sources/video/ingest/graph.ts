...
import type { VideoIngestStreamEvent, VideoTextChunk } from "../types";
import { chunkTranscript } from "./chunking";
import { getTranscript } from "./transcript";
import { VideoIngestStateAnnotation, type VideoIngestState } from "./state";

function emit(config: LangGraphRunnableConfig, event: VideoIngestStreamEvent) {
  config.writer?.(event);
}
...
function embedTextForChunk(chunk: VideoTextChunk): string {
...
async function extractTranscriptNode(
  ...
  try {
    ...
    console.error("[video/ingest] failed to extract transcript:", error);
    const message =
...
function routeAfterExtract(state: VideoIngestState) {
...
async function embedAndStoreNode(
  ...
  try {
    ...
    console.error("[video/ingest] failed to embed/store chunks:", error);
    const message =
...
export function buildVideoIngestGraph() {
  const graph = new StateGraph(VideoIngestStateAnnotation)
